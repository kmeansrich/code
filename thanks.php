<?php 
$noindex = 'yes';
$metatitle = "Order Has Been Done - MyphotoPrint.in";
$metadesc = "Your Myphotoprint Order Has Been Done";
$metaurl = "https://myphotoprint.in/thanks.php";
include('header.php');
require('config.php');
...
if(!empty($helper->get_session('USER_ID'))){
    ...
    if($orderPayMode == 'ccavenue' || $orderPayMode == 'razorpay' || $orderPayMode == 'partialpayment' ||  $orderPayMode == 'cod'){  
        if(!empty($helper->get_session('orderId'))){
            ...
            if(isset($_POST['razorpay_payment_id']) && isset($_POST['razorpay_order_id'])){
                ...
                elseif($orderPayMode == 'ccavenue' || $orderPayMode == 'razorpay'){
                    ...
                    else{ $statusArray = [["datetime" => $date, "status"  =>  "$failurestatus", "comment" => " "]]; }
                    $statusArray = json_encode($statusArray);
                    $dataaraay2 = ['see_uop_status' => $statusArray];
    ...
    $getuserConfirmedOrders = $see->getrows("see_orders","see_uo_id= '$lastOrderId' ORDER BY see_uo_id DESC");  ?>
	                ...
	                <div class="col-sm-12 col_p">
	                    <div class="table table-bordered table-condensed table_p">
                            ...
                            <div class="upper_case">
                                ...
                                <span class="hide_border"><?=$orderPayMode;?></span>
                            </div>
                            <div class="upper_case">
                                <span><b> Phone : </b></span>
        ...
        curl_setopt_array($fbcurl, array(
            ...
            CURLOPT_HTTPHEADER => array("cache-control: no-cache","Accept: application/json"),
        ));
        $fbresponse = curl_exec($fbcurl);
        $fberr = curl_error($fbcurl);
        if($fberr){ curl_getinfo($fbcurl); }
    ?>
    <script> gtag('event', 'conversion', { 'send_to': 'AW-982171696/SRrNCPab75EBELCAq9QD', 'transaction_id': '' }); </script>
    <?php 
    }
...
<script>
    ...
    document.getElementById('popup1_close').onclick = function(){ ele1.classList.remove('appear'); }
    gtag('event','conversion',{ 'send_to': 'AW-925593001/kocTCLiP-LgBEKnbrbkD','transaction_id': '' });
    gtag('event','conversion',{ 'send_to': 'AW-982171696/SRrNCPab75EBELCAq9QD','transaction_id': '' });
    scq('Purchase', 'pre_defined');
</script>
<?php
}
